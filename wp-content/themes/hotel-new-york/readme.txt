=== Hotel New York ===
Contributors: webdzier
Requires at least: 4.4
Tested up to: 5.7.2
Requires PHP: 5.6
Stable tag: 1.1.6
Version: 1.1.6
License: GPLv3 or later
License URI: https://www.gnu.org/licenses/gpl-3.0.html
Tags: one-column, two-columns, right-sidebar, custom-colors, custom-menu, sticky-post, threaded-comments, custom-header, featured-images, blog

== Copyright ==

Hotel New York WordPress Theme, Copyright 2020 Webdzier
Hotel New York is distributed under the terms of the GNU GPL

Hotel Galaxy WordPress Theme, Webdzier. Hotel Galaxy WordPress Theme is distributed under the terms of the GNU GPL

== Description ==

Hotel New York is a hotel based WordPress theme which is clean and responsive design. It is used to hotel, restaurant, eatery, food joint, cuisine, hospitality business, recipe, cafe, lodge and others. It is mobile friendly and has a very nice animated homepage. It is easily to setup and customize using Customizer API.

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

----------------------------------------------------------------------------------------------
License and Copyrights for Resources used in Hotel Sydney WordPress Theme
----------------------------------------------------------------------------------------------

* Font Awesome CSS
Copyright 2015  Dave Gandy.
License - http://fontawesome.io/license (Font: SIL OFL 1.1, CSS: MIT License)
Source:https://fortawesome.github.io/Font-Awesome/

* Bootstrap CSS 
Copyright 2011-2015 Twitter, Inc.
Licensed under MIT and GPL license (https://github.com/twbs/bootstrap/blob/master/LICENSE)
Source:http://getbootstrap.com

* Fonts library 
Font Fira Sans and Bitter - SIL Open Font License, 1.1
License URI: http://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL
Source: https://www.google.com/fonts

* Bootstrap js
Copyright 2011-2015 Twitter, Inc.
Licensed under MIT and GPL license :http://opensource.org/licenses/MIT
Source:http://getbootstrap.com

* Owl Carousel 2.3.4
Owl Carousel 2.3.4 - by @David Deutsch - https://github.com/OwlCarousel2
License - MIT License(https://github.com/OwlCarousel2/OwlCarousel2/blob/master/LICENSE)

* WOW Js
OW (https://wowjs.uk/)
Copyright (c) 2016 Thomas Grainger.
License - MIT License (https://github.com/graingert/WOW/blob/master/LICENSE)

* Animate CSS
Source:https://animate.style/
Copyright (c) 2020 Daniel Eden
License - MIT License (https://github.com/animate-css/animate.css/blob/main/LICENSE)

* Custom js
custom.js jquery
Copyright 2016 webdzier

* Customizer control js
customizer-controls.js jquery
Copyright 2020 webdzier

* Customizer preview
customizer-live.js jquery
Copyright 2020 webdzier

* Slider Section Image
URL: https://stocksnap.io/photo/women-working-2VU3XNN1MA
Source: https://stocksnap.io
License: CC0 Public Domain

* Logo Image
Copyright 2020 webdzier
Licensed under MIT and GPL license

* Close Image
Copyright 2020 webdzier
Licensed under MIT and GPL license

== Changelog ==

= 1.1.6 =
* Tested up to WordPress 5.7.2

= 1.1.5 =
* Tested by WordPress 5.6

= 1.1.4 =
* New WordPress tested.

= 1.1.3 =
* style.css file updated.

= 1.1.2 =
* Resolved text domain issue

= 1.1.1 =
* Initial upload

**Images**

All images from pixabay (https://pixabay.com) are licensed under  Creative Commons CC0 Public Domain.
All other images  are own creation licensed under the [GNU GPL](http://www.gnu.org/licenses/gpl-3.0.txt), version 3 or later.


*  https://pixabay.com/en/dinner-table-restaurant-dining-444434/

*  https://pixabay.com/en/hotel-room-curtain-green-furniture-1979406/

*  https://pixabay.com/en/travel-hotel-rooms-hotel-room-bed-1677347/

*  https://pixabay.com/en/villa-interior-hotel-bedroom-1737168/

